...
```
WARNING: Found 1058 files not referenced in any member of index_sheets -- these files may represent renames, files for future releases, etc...
...
│ {["HPRC_PLUS_nanopore_misc_R2_data_table__final.csv", "manifest"],7}      │
...
│ {["manifest", "HPRC_PLUS_nanopore_misc_R2_data_table__final.csv"],2}      │
│ {["WUSTL_HPRC_HiFi_Year3_TopUp_data_table__final.csv"],2}                 │
│ {["UW_HPRC_HiFi_Y1_data_table__final.csv"],2}                             │
└───────────────────────────────────────────────────────────────────────────┘
All other stats below EXCLUDE the not-in-any-member-of-index_sheets oddities.
...
* 4130 have an NCBI SRA accession

The 1918 files that are in an index_sheet, but do not have an NCBI accession, have these values for production (this might help you find 'collections' that aren't in the HPRC metadata repo or are problematic)
┌───────────────────────────────────────┐
│ production                            │
│ ---                                   │
│ struct[2]                             │
